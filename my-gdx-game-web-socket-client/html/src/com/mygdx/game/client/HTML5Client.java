...
public class HTML5Client implements ApplicationListener {
	...
	private static int x_card = 0; // Used to send the coordinates of card
	private static int y_card = 0;
	// position
	private int last_x = 0, last_y = 0; // Used to store the last sent
										// coordinates
	...
	public void create() {
		...
		clientMSG = new ClientMSG("127.0.0.1", 8077);
		...
		// texture.setFilter(TextureFilter.Linear, TextureFilter.Linear); //Use
		// Texture Filter (may be slow).
	...
	public void render() {
		...
		camera.update(); // Is a good practice to update the camera in every
							// frame, but the response time is worse
